...
exports.getBudgetItemsFromDB = exports.deleteBudgetFromDB = exports.updateBudgetSpent = exports.addBudgetsToDB = exports.getBudgetsFromDB = void 0;
const db_1 = __importDefault(require("../db"));
// Fetch all budgets with specific columns only
const getBudgetsFromDB = () => db_1.default
    .query('SELECT id, title, budget, spent, (budget - spent) AS remaining, created_at FROM budgets ORDER BY created_at DESC')
...
const updateBudgetSpent = (id, spent) => db_1.default
    .query(`UPDATE budgets 
       SET spent = spent + $1 
       WHERE id = $2 
       RETURNING id, title, budget, spent, (budget - spent) AS remaining, created_at`, [spent, id])
    .then((result) => result.rows[0]);
exports.updateBudgetSpent = updateBudgetSpent;
// ✅ Delete a budget and return deleted budget details
const deleteBudgetFromDB = (id) => db_1.default
    .query('DELETE FROM budgets WHERE id = $1 RETURNING id, title, budget, spent, (budget - spent) AS remaining, created_at', [id])
    .then((result) => result.rows[0]);
exports.deleteBudgetFromDB = deleteBudgetFromDB;
// ✅ Fetch items for a specific budget
const getBudgetItemsFromDB = (budgetId) => db_1.default
    .query(`SELECT id, budget_id, description, amount, created_at FROM budget_items WHERE budget_id = $1 ORDER BY created_at DESC`, [budgetId])
    .then((result) => result.rows);
